...
samples = np.loadtxt('train-data/generalsamples5.data',np.uint8)
responses = np.loadtxt('train-data/generalresponses5.data',np.uint8)
print(samples.shape)
...
def solve_sudoku_ocr(src, crossing_points):
    ...
    for i, pos in enumerate([pos for pos in range(89) if (pos + 1) % 10 != 0]):
        ...
        #print(transformed)
        predictedno = knn.predict(sample) - 48
        #print(clf.predict(sample))
        print(predictedno)

        #print(sample.shape)
        #example_measures = sample.reshape(len(sample), -1)
